...
import { firestore } from "../js/firebase-config.js";
		...
		const eventLeadsCollection = collection(firestore, "eventLeads");
		...
		function populateLeadsTable(leads) {
			...
			leads.forEach((lead) => {
				...
				designationCell.textContent = lead.eventName;
				row.appendChild(designationCell);

				const paymentCell = document.createElement("td");
				paymentCell.textContent = "Completed";
		...
		async function readAllLeads() {
			try {
				...
				const querySnapshot = await getDocs(
					query(eventLeadsCollection, orderBy("timestamp"))
				);
		...
		designationSearchInput.addEventListener("input", readLeadsWithEvents);

		async function readLeadsWithEvents() {
			try {
				clearLeadsTable();
